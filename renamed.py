...
#Paragraphs Definitons to be appended (Considering the number paragraph model as follows = numberParagraph - 1)
    ...
    #Adding NAME PARAGRAPH
    paragraphName = paragraph_prior2.insert_paragraph_before()
    paragraphNameInput = paragraphName.add_run(f'{listParagraphName[contractsNum]}')
    paragraphName.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    paragraphName.paragraph_format.right_indent = Inches(3.15)
    paragraphName.bold = False

    #Adding RENDA PARAGRAPH
    paragraphRenda = paragraph_prior3.insert_paragraph_before()
    toBold = paragraphRenda.add_run('§ 2º')
    toBold.bold = True
